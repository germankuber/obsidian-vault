...
- **[[Derived Reranking]]** — rerankea *después* de recuperar, solo entre el top-K. Apunta a **precisión**: refina lo ya pescado, minimiza falsos positivos. "No agranda la red; inspecciona lo que ya cayó." Usa métodos deterministas ([[BM25]], TF-IDF) o semánticos ([[Bi-Encoder]], [[Cross-Encoder]]).
- **[[Hybrid Reranking]]** — rerankea *durante* la recuperación, fusionando señales de varios retrievers (sparse + dense + dominio). Apunta a **recall**: expande el pool, reduce falsos negativos. Su método de consenso típico es [[Reciprocal Rank Fusion]].
